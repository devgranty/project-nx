...
if(!empty($_GET['p'])){
	...
	$selectQuery = $db->selectQuery('__posts', ['id', 'user_id', 'title', 'category', 'thumbnail', 'article', 'source', 'date_added'], ['WHERE' => ['id' => $post_id]]);
	...
	// let us make sure you are the owner of this.
	if(Session::get('uid') != $data['user_id']) Router::redirect('403.php');
		...
		if(Hash::compareHashes(Hash::hashUseHmac('sha256', get_page_name(), Session::get('_token')), $_POST['form_token'])){
			...
			if(!$_err){
				$updateQuery = $db->updateQuery('__posts', [
					...
					'source' => sanitize_input($_POST['source'])], ['id' => $post_id]);
...
$post = post_values(['title' => $data['title'], 'category' => $data['category'], 'article' => $data['article'], 'source' => $data['source'], 'date_added' => Datetime::setDateTime($data['date_added'])], 'update_article');
?>
	...
	<body>
		...
		<div class="min-vh-100">
			...
			<div class="container mt-5">
				...
				<div class="modal fade" id="thumbnailModal" tabindex="-1" role="dialog" aria-labelledby="thumbnailModallLabel" aria-hidden="true">
