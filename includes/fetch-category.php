...
if(!empty($_POST['page']) && !empty($_POST['category'])):
    ...
    $selectQuery = $db->query("SELECT __posts.id, __posts.slug, __posts.title, __posts.section, __posts.thumbnail, __posts.article, __posts.date_added, __users.uname, __users.photo FROM __posts JOIN __users ON __posts.user_id = __users.id WHERE __posts.category = ? AND __posts.status = ? ORDER BY __posts.id DESC LIMIT $position, $item_per_page", [$category, 'approved']);
