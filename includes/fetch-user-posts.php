...
if(!empty($_POST['page'])):
    ...
    $selectQueryUserPost = $db->query("SELECT __posts.id, __posts.slug, __posts.title, __posts.section, __posts.thumbnail, __posts.article, __posts.date_added, __users.uname, __users.photo FROM __posts JOIN __users ON __posts.user_id = __users.id WHERE __posts.user_id = ? AND __posts.status = ? ORDER BY __posts.id DESC LIMIT $position, $item_per_page", [$user_id, 'approved']);

    if($selectQueryUserPost->row_count() > 0):

        while($data = $selectQueryUserPost->results()):
            ...
            switch($data['section']){
                case 'pinned':
                    $section = '<span class="s-bull" aria-hidden="true">&bull;</span> <i class="fas fa-thumbtack" aria-hidden="true" data-toggle="tooltip" title="Pinned post"></i>';
                break;
                case 'trending':
                    $section = '<span class="s-bull" aria-hidden="true">&bull;</span> <i class="fas fa-fire-alt" aria-hidden="true" data-toggle="tooltip" title="Trending post"></i>';
                break;
                default:
                    $section = '';
                break;
